...
export default function KundenstimmenPage() {
  ...
  return (
    <div className="wrap" style={{ padding: "56px 0 80px" }}>
      ...
      {schnitt && (
        <p style={{ color: "var(--text-dim)", marginBottom: 32, display: "flex", alignItems: "center", gap: 8 }}>
          <Sterne n={Math.round(schnitt)} /> {schnitt} / 5 · {bewertungen.length} Bewertung(en)
        </p>
      )}
      ...
      {loading ? (
        ...
        <p style={{ color: "var(--text-dim)" }}>Noch keine Bewertungen vorhanden.</p>
      ) : (
